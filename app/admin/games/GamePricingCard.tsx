      ...
      <div className="hairline rounded-2xl bg-panel overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-4 border-b border-line">
          <div className="min-w-0">
            <h3 className="display text-base font-extrabold text-ink truncate">{game.name}</h3>
            <p className="text-[11px] text-muted mt-0.5">{game.range_label}</p>
          </div>
          <Toggle
            checked={isActive}
        ...
        <div className="divide-y divide-line">
          {items.length === 0 && (
            <p className="text-xs text-muted py-6 text-center">Belum ada nominal</p>
          )}
          {items.map((item) => (
              ...
              {editing === item.id ? (
                <>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="shrink-0 w-9 h-9 flex items-center justify-center rounded-lg bg-paper-2 text-muted hover:text-white hover:bg-line transition disabled:opacity-50"
                    title="Batal"
                  >
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="shrink-0 w-9 h-9 flex items-center justify-center rounded-lg bg-paper-2 text-muted hover:text-red-400 hover:bg-red-500/10 transition disabled:opacity-50"
                    title="Hapus"
                  >
